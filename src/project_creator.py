    ...
    def create_error_txt_file_if_errors_occur(self):
        if len(self.errors) != 0:
            if len(self.errors) == 1:
                print("Warning: An error has occurred in the creation of your project. Please check the 'agps_errors.txt' file.")
            elif len(self.errors) > 1:
                print("Warning: Multiple errors have occurred in the creation of your project. Please check the 'agps_errors.txt' file.")

            with open("agps_errors.txt", "w") as f:
                f.write("# ERRORS #\n\n")
                f.writelines([f"{num + 1}) {error}\n" for num, error in enumerate(self.errors[:-1])])
                f.write(f"{len(self.errors)}) {self.errors[-1]}")